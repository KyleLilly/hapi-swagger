...
internals.docs = function (settings) {
    ...
    return {
        ...
        handler: function (request, reply) {
            ...
            // request.server.info is not reliable if there are many connections on the same server
            if(request.server.info){
                var defaultServerProtocol = request.server.info.protocol;
            }
...
internals.getRoutesData = function (routes, settings) {
    ...
    routes.forEach(function (route) {
        ...
        if( Hoek.reach(routeOptions, 'validate.query') ){
            routeData.queryParams =  internals.paramObjectToArray( Hoek.reach(routeOptions, 'validate.query') );
        }
        if( Hoek.reach(routeOptions,'validate.params') ){
            routeData.pathParams =  internals.paramObjectToArray( Hoek.reach(routeOptions,'validate.params') );
        }
        if( Hoek.reach(routeOptions, 'validate.payload') ){
            routeData.payloadParams =  internals.paramObjectToArray( Hoek.reach(routeOptions, 'validate.payload') );
        }
...
// turn validate into array used internally
internals.paramObjectToArray = function (obj) {
     var out = [];
     for (var key in obj) {
...
internals.appendReplyContext = function ( server, settings ) {
    server.ext('onPostHandler', function (request, reply) {
        ...
        if (response.variety === 'view') {
            ...
            // To adapt to hapi prefix - uses appendedPrefix make sure its only added once
            if (settings.appendedPrefix === false){
